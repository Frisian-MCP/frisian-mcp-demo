...
# WHY THIS EXISTS
#
# The golden dump assigns table ownership to the `paperless` role. The postgres
...
# ${POSTGRES_USER:-paperless}. Compose cannot assert its own values, so the
# check lives here, where every path into this image passes through it.
set -euo pipefail

EXPECTED_ROLE="paperless"
...
if [ "${POSTGRES_USER:-}" != "$EXPECTED_ROLE" ]; then
  ...
  echo >&2
  echo "Set POSTGRES_USER=${EXPECTED_ROLE} (and PAPERLESS_DBUSER to match)."    >&2
  echo "============================================================" >&2
  exit 1
fi
