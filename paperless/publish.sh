#!/usr/bin/env bash
#
# Publish the Paperless demo image PAIR to GHCR.
#
...
APP_IMAGE="${REGISTRY}/demo-paperless"
DB_IMAGE="${REGISTRY}/demo-paperless-db"
...
# ── Both halves of the estate, or neither ────────────────────────────────────
#
# This host's estate is split across the two images: the db image carries the
# SQL, the app image carries the files that SQL points at. Checking only the
# dump would let a publish through that produces a document list where every
# download 404s — which looks like a frisian-mcp bug and is not one.
missing=()
[ -f db/demo.sql.gz ]      || missing+=("db/demo.sql.gz")
[ -f estate/media.tar.gz ] || missing+=("estate/media.tar.gz")
if [ ${#missing[@]} -gt 0 ]; then
  echo "ERROR: the estate artifacts are missing:" >&2
  for m in "${missing[@]}"; do echo "    $m" >&2; done
  echo >&2
  echo "Both are produced together by one command:" >&2
  if [ -n "$WHEEL" ]; then
  ...
  echo >&2
  echo "Do not produce one without the other. They are halves of one artifact." >&2
  exit 1
fi
echo "  estate      db/demo.sql.gz $(wc -c < db/demo.sql.gz | tr -d ' ') bytes"
echo "              estate/media.tar.gz $(wc -c < estate/media.tar.gz | tr -d ' ') bytes"
...
FIRST PUBLISH — the packages stay CLOSED. Manual steps:
  ...
  Then, from a fresh clone:
       cd paperless && DEMO_TAG=${DEMO_TAG} docker compose up
