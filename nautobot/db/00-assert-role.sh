...
# WHY THIS EXISTS
#
# The golden dump contains 272 `ALTER TABLE ... OWNER TO nautobot;` statements.
# The postgres entrypoint restores .sql.gz with ON_ERROR_STOP, so a role that
# does not match makes the restore FATAL rather than merely noisy — and the
# stack then comes up with a database that is empty or half-populated for a
# reason buried hundreds of lines into a container log.
#
...
# ${POSTGRES_USER:-nautobot}. Compose cannot assert its own values, so the
# check lives here, where every path into this image passes through it.
set -euo pipefail

EXPECTED_ROLE="nautobot"
...
if [ "${POSTGRES_USER:-}" != "$EXPECTED_ROLE" ]; then
  ...
  echo >&2
  echo "Set POSTGRES_USER=${EXPECTED_ROLE} (and NAUTOBOT_DB_USER to match)."    >&2
  echo "============================================================" >&2
  exit 1
fi
