...
#
# The secret is HMAC'd by `_hmac_secret`, which prefers FRISIAN_MCP_HMAC_KEY
# and falls back to SECRET_KEY — the same split as F1. (The model's help_text
# says "keyed by SECRET_KEY", which is wrong; the code prefers the HMAC key.)
# Because we pin the HMAC key, a baked secret survives per-deployment
# SECRET_KEY rotation. Mint it under the wrong key and browser-connect breaks
# silently, exactly like the static tokens.
...
# Exact-match is required (SEC-2), so these are literal, not patterns.
#
# The loopback entries are verified against this stack's own default binding.
# The claude.ai entry is the connector's published callback and is NOT
# verifiable from here — if a browser connect fails with an invalid redirect,
# this list is the first thing to correct, and it is a one-line edit plus a
# re-run. Listed rather than omitted because omitting it guarantees the flow is
# dead for the audience most likely to try it.
OAUTH_REDIRECT_URIS = [
